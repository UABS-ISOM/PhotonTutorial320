        ...
        void Start()
        {
            ...
            else
            {
                if (VRPlayerManager.LocalPlayerInstance == null)
                {
                    ...
                    GameObject _player = PhotonNetwork.Instantiate(this.playerPrefab.name, new Vector3(0f,5f,0f), Quaternion.identity, 0); //we can swap this out here for VR capable model
                    _player.GetComponent<VRPlayerManager>().playerCameraToFollow = cameraToFollow;
                }
                else
                {
